      ...
      <div className="px-5 py-3.5 flex items-center gap-4">
        ...
        {!isGlobal && (
          <div className="flex items-center gap-1 shrink-0">
            <button
              onClick={() => onEdit(q)}
              className="w-7 h-7 flex items-center justify-center rounded-lg bg-white hover:bg-sky-50 text-slate-500 hover:text-sky-600 transition-colors border border-slate-100 hover:border-sky-200"
              title="Modifier"
            >
                  ...
                  {q.options.map((opt, i) => (
                    <div key={i} className="flex items-center gap-2.5">
                      {q.type === 'multiple' ? (
                        ...
                        <div className="w-3.5 h-3.5 rounded-full border-[1.5px] border-sky-300 shrink-0" />
                      )}
...
const inputClass =
  "w-full px-3.5 py-2.5 bg-white border border-slate-200 rounded-lg text-[12px] font-semibold text-slate-800 placeholder:text-slate-400 outline-none transition-all focus:border-sky-300 focus:ring-2 focus:ring-sky-100";
...
const AttributionModal = ({ isOpen, onClose, patientId, medecinId }) => {
  ...
  const handleSubmit = async () => {
    if (!frequence) { setError('Veuillez choisir une fréquence.'); return; }
    setLoading(true);
    setError(null);
    try {
      await axiosInstance.post('/questionnaire/attribuer', {
        patientId, medecinId, frequence, dateFin: dateFin || null,
      });
      setSuccess(true);
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          >
            <div className="px-6 py-5 flex items-start justify-between border-b border-slate-100 bg-slate-50/50">
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-xl bg-white border border-slate-100 flex items-center justify-center shadow-sm">
                  <Send size={16} className="text-pink-500" strokeWidth={2.5} />
                </div>
                <div>
            ...
            <div className="px-6 py-6 flex flex-col gap-5">
              ...
              <div className="flex items-start gap-3 px-3 py-3 bg-pink-50 border border-pink-100 rounded-lg">
                <ClipboardList size={14} className="text-pink-400 shrink-0 mt-0.5" />
    ...
    <div className="max-w-full mx-auto space-y-4 animate-in fade-in duration-500 min-h-screen p-6" style={{ backgroundColor: '#fafbfc' }}>
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="flex items-center gap-5">
          ...
          <div>
            <h1 className="text-2xl font-black text-slate-900 tracking-tight">Questionnaires Patient</h1>
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="inline-flex items-center gap-2 px-5 py-3 bg-pink-400 hover:bg-pink-500 text-white text-[10px] uppercase font-black tracking-[0.15em] rounded-xl transition-all shadow-md"
          >
      ...
      <div className="bg-white rounded-xl border border-slate-100 shadow-md overflow-hidden">
        <div className="px-5 py-3 flex items-center justify-between border-b border-slate-100 bg-slate-50/50">
          <div className="flex items-center gap-2">
            ...
            <span className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Questions de Base (Globales)</span>
          </div>
        ...
        {globalQuestions.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-12 gap-3">
            ...
            <p className="text-[11px] font-black text-slate-500 uppercase tracking-widest">Aucune question globale disponible</p>
          </div>
        ) : (
          <div>
            {globalQuestions.map((q, idx) => (
              <QuestionRow key={q.id} q={q} idx={idx} isLast={idx === globalQuestions.length - 1} isGlobal={true} />
            ))}
          </div>
        )}
      </div>
      ...
      <div className="bg-white rounded-xl border border-slate-100 shadow-md overflow-hidden">
        <div className="px-5 py-3 flex items-center justify-between border-b border-slate-100 bg-slate-50/50">
          <div className="flex items-center gap-2">
            ...
            <span className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Questions Personnalisées</span>
          </div>
          <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest tabular-nums">
        ...
        {customQuestions.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-12 gap-3">
            ...
            <p className="text-[11px] font-black text-slate-500 uppercase tracking-widest">Aucune question personnalisée</p>
            <button
              onClick={openAddModal}
          ...
          <div>
            {customQuestions.map((q, idx) => (
              <QuestionRow
                ...
                isGlobal={false} onEdit={openEditModal} onDelete={confirmDelete}
              />
            ))}
          </div>
        )}
            ...
            >
              <div className="px-6 py-5 flex items-start justify-between border-b border-slate-100 bg-slate-50/50">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-xl bg-white border border-slate-100 flex items-center justify-center shadow-sm">
                    {currentQuestion
                      ? <Edit3 size={16} className="text-sky-500" strokeWidth={2.5} />
                      : <Plus size={16} className="text-sky-500" strokeWidth={2.5} />
                    }
                  </div>
                  <div>
                    <h2 className="text-base font-black text-slate-900 tracking-tight leading-none mb-1.5">
                ...
                <div>
                  <div className="flex items-center justify-between mb-2.5">
                    ...
                    <button
                      ...
                      className="inline-flex items-center gap-1.5 px-2.5 py-1 bg-slate-50 hover:bg-sky-50 text-slate-600 hover:text-sky-700 rounded-md text-[9px] font-black uppercase tracking-widest border border-slate-200 hover:border-sky-200 transition-colors"
                    >
              ...
              <div className="px-6 py-6">
                <div className="flex items-start gap-4 mb-5">
                  ...
                  <div className="min-w-0 pt-1">
                    <h3 className="text-base font-black text-slate-900 tracking-tight mb-1.5">Confirmer la suppression</h3>
                    <p className="text-[9px] font-bold text-rose-600 uppercase tracking-widest">Action irréversible</p>
                  </div>
                </div>
                ...
                {questionToDelete && (
                  <div className="bg-white border border-slate-200 rounded-lg p-3 mb-5">
                    <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest mb-1.5">Question concernée</p>
                    <p className="text-[12px] font-bold text-slate-800 line-clamp-2">{questionToDelete.text}</p>
                  </div>
                )}
