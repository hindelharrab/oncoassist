...
import {
  ...
  Building2, Database, Smartphone, Camera
} from 'lucide-react';
...
const SettingsPage = () => {
  ...
  const loadProfil = async () => {
    ...
    photoProfil: medecin.photoProfil   || '',
        ...
        if (medecin.photoProfil) {
          ...
          // Synchronise le contexte dès le chargement
          const updatedUser = { ...user, photoProfil: photoUrl };
          login(updatedUser);
        }
    ...
    loadProfil();
  }, [user?.id]); // ← user?.id uniquement pour éviter les boucles infinies
  ...
  const handlePhotoChange = (e) => {
    ...
    // ← Met à jour le contexte immédiatement
    // pour que navbar/sidebar reflète le changement sans attendre la sauvegarde
    const updatedUser = { ...user, photoProfil: localPreview };
    login(updatedUser);
  };
  ...
  const handleSave = async () => {
    try {
      ...
      // Mettre à jour le preview et doctorInfo
      ...
      alert(error.response?.data?.message || error.message || 'Impossible de sauvegarder.');
    }
  };
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-x-12 gap-y-10">
                  ...
                  <div className="lg:col-span-2 flex items-center gap-8 pb-8 border-b border-gray-50 dark:border-gray-800">
                    ...
                    <div>
                      ...
                      {/* Badge "photo non sauvegardée" */}
                      {photoFile && (
                        <p className="text-[9px] text-amber-500 font-bold mt-2 uppercase tracking-widest">
