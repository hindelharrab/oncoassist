...
import {
  ...
  Clock, User, Stethoscope, X, CheckCircle2,
...
/* ════════════════════════════════════════
   PALETTE THÈME — violet / rose / slate
═══════════════════════════════════════ */
const STATUT_CFG = {
  CONFIRME:   {
    label:     'Confirmé',
...
/* ─── Input style ─── */
const inputClass =
  'w-full h-11 px-4 bg-slate-50 border border-slate-200 rounded-xl text-[13px] font-medium text-slate-800 outline-none focus:border-violet-400 focus:ring-2 focus:ring-violet-400/10 transition-all placeholder:text-slate-300';
...
const ModalRdv = ({ rdv, onClose, onSave }) => {
  ...
  const isEdit = !!rdv;
  const canSave = form.patient && form.medecin && form.date && form.heure && form.motif;
      ...
      >
        ...
        <div className="flex items-center justify-between px-6 py-4 border-t border-slate-100 bg-slate-50/50">
          ...
          <button
            ...
            style={{ background: canSave ? 'linear-gradient(135deg,#7c3aed,#db2777)' : '#94a3b8' }}
          >
...
const VueSemaine = ({ rdvList, offsetSemaine, filtreMedecin, onEdit }) => {
  ...
  const rdvDuJour = (jourOffset) =>
   ...
   const matchMedecin = filtreMedecin === 'Tous' || r.medecin.toLowerCase().includes(filtreMedecin.toLowerCase());
        ...
        return (
          <div key={i} className="space-y-2 min-h-[200px]">
            {rdvs.length === 0 ? (
              ...
              rdvs.map((rdv) => {
                const c = STATUT_CFG[rdv.statut] || STATUT_CFG.EN_ATTENTE;
                return (
                  <button
                    key={rdv.id}
                    onClick={() => onEdit(rdv)}
...
const VueListe = ({ rdvList, filtreMedecin, onEdit }) => {
 const filtered = rdvList.filter(
  (r) => filtreMedecin === 'Tous' || r.medecin.toLowerCase().includes(filtreMedecin.toLowerCase())
);
            ...
            {rdvs.map((rdv) => {
              ...
              return (
                <motion.div
                  ...
                  className="flex items-center gap-4 p-4 bg-white rounded-2xl border border-slate-100 hover:border-violet-200 hover:shadow-sm transition-all group cursor-pointer"
                  onClick={() => onEdit(rdv)}
                >
                  {/* Heure */}
    ...
    <div className="space-y-5 pb-8">
      ...
      <motion.div {...fadeUp(0)} className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <motion.button
          ...
          className="flex items-center gap-2 h-10 px-5 rounded-xl text-[12px] font-bold text-white shadow-sm shrink-0 relative overflow-hidden"
          style={{ background: '#0f172a' , boxShadow: '0 4px 16px rgba(124,58,237,0.22)' }}
        >
      ...
      <motion.div {...fadeUp(0.1)} className="flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-3">
          {/* Filtre médecin */}
          {/* Filtre médecin — recherche par nom */}
...
onChange={(e) => setFiltreMedecin(e.target.value === '' ? 'Tous' : e.target.value)}
    className="h-9 pl-9 pr-4 bg-white border border-slate-200 rounded-xl text-[12px] font-medium text-slate-700 outline-none focus:border-violet-400 focus:ring-2 focus:ring-violet-400/10 transition-all placeholder:text-slate-300 w-52"
  />
  {filtreMedecin !== 'Tous' && (
    <button
    ...
    >
      ...
      {/* ── Légende ── */}
      <motion.div {...fadeUp(0.12)} className="flex flex-wrap items-center gap-5 px-1">
        {Object.values(STATUT_CFG).map(({ label, dot }) => (
      ...
      >
        <AnimatePresence mode="wait">
          {vue === 'semaine' ? (
            ...
            >
              <VueSemaine rdvList={rdvList} offsetSemaine={offsetSemaine} filtreMedecin={filtreMedecin} onEdit={handleEdit} />
            </motion.div>
          ) : (
            <motion.div key="liste"
              ...
              exit={{ opacity: 0, x: -10 }} transition={{ duration: 0.18 }}
            >
              <VueListe rdvList={rdvList} filtreMedecin={filtreMedecin} onEdit={handleEdit} />
            </motion.div>
          )}
        </AnimatePresence>
      </motion.div>
