...
import {
  Users, UserPlus, Search, Filter, Printer, Eye, Edit2,
  X, CheckCircle2, AlertCircle, Clock, Phone, Mail,
...
const StatutBadge = ({ statut }) => {
  const map = {
    Suivi:   { className: 'bg-gray-100 text-gray-900 border-gray-200',  icon: CheckCircle2 },
    Nouveau: { className: 'bg-pink-50 text-pink-600 border-pink-200',   icon: AlertCircle },
    Clôturé: { className: 'bg-gray-50 text-gray-500 border-gray-200',   icon: Clock },
  };
...
const StatCard = ({ icon: Icon, label, value, color }) => {
  const colors = {
    ...
    dark:  { bg: 'bg-gray-100', icon: 'text-gray-700' },
...
/* ════════════════════════════════════════
   LIGNE PATIENT
═══════════════════════════════════════ */
const PatientRow = ({ patient, index, onView, onEdit, onPrint }) => (
  <motion.tr
  ...
  >
    ...
    <td className="py-3.5 px-5">
      <div className="flex items-center gap-3">
        <div className="w-9 h-9 rounded-xl bg-gray-200 flex items-center justify-center text-[11px] font-black text-gray-700 shrink-0">
        ...
        <div>
          ...
          <p className="text-[11px] text-gray-500 font-medium mt-0.5">{patient.id}</p>
        </div>
      </div>
    </td>
    ...
    <td className="py-3.5 px-5 hidden md:table-cell">
      <span className="text-[13px] font-semibold text-gray-800">{patient.age} ans</span>
    </td>
    ...
    <td className="py-3.5 px-5 hidden lg:table-cell">
      <div className="flex flex-col gap-0.5">
        <div className="flex items-center gap-1.5 text-gray-700">
          ...
          <span className="text-[12px] font-medium">{patient.telephone}</span>
        </div>
        <div className="flex items-center gap-1.5 text-gray-500">
          <Mail size={11} strokeWidth={1.8} />
          <span className="text-[11px] truncate max-w-[140px]">{patient.email}</span>
        </div>
      </div>
    </td>
    ...
    <td className="py-3.5 px-5 hidden sm:table-cell">
      <div className="flex items-center gap-2">
        <div className="w-7 h-7 rounded-lg bg-gray-200 flex items-center justify-center text-gray-700 text-[9px] font-black shrink-0">
          DR
        </div>
    ...
    <td className="py-3.5 px-5 hidden xl:table-cell">
      <span className="text-[12px] font-medium text-gray-700">{patient.derniereConsultation}</span>
    </td>
    ...
    <td className="py-3.5 px-5 text-right">
      <div className="flex items-center justify-end gap-1.5 opacity-0 group-hover:opacity-100 transition-opacity">
        ...
        <button
          ...
          title="Voir dossier"
          className="w-8 h-8 rounded-lg bg-black flex items-center justify-center text-white hover:bg-pink-500 transition-colors"
        >
          <Eye size={13} />
        </button>
      </div>
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <motion.div
        ...
        className="relative w-full max-w-2xl bg-white rounded-2xl shadow-xl border border-gray-100 overflow-hidden max-h-[90vh] flex flex-col"
      >
        {/* En-tête */}
        <div className="flex items-center justify-between px-6 py-5 border-b border-gray-100 shrink-0">
          ...
          <button onClick={onClose} className="w-8 h-8 rounded-xl border border-gray-200 flex items-center justify-center text-gray-400 hover:bg-gray-50 transition-colors">
            <X size={16} />
          </button>
        </div>
        ...
        <div className="flex items-center justify-between px-6 py-4 border-t border-gray-100 bg-gray-50/50 shrink-0">
          <button onClick={onClose} className="text-[12px] font-medium text-gray-500 hover:text-gray-700 transition-colors">
            Annuler
          </button>
          <button
            onClick={handleSubmit}
            disabled={!form.nom || !form.prenom || !form.medecinRef}
            className="flex items-center gap-2 h-10 px-6 bg-black text-white rounded-xl text-[13px] font-semibold hover:bg-pink-500 disabled:opacity-40 disabled:cursor-not-allowed transition-colors"
          >
...
═══════════════════════════════════════ */
const PATIENTS_MOCK = [
  { id: '#P-9821', nom: 'Bennani',   prenom: 'Salma',   age: 42, telephone: '06 11 22 33 44', email: 's.bennani@mail.com',  medecinRef: 'El Harrab', derniereConsultation: '14 mai 2026', statut: 'Suivi'   },
  { id: '#P-5512', nom: 'Alami',     prenom: 'Yassir',  age: 35, telephone: '06 55 66 77 88', email: 'y.alami@mail.com',    medecinRef: 'Benali',    derniereConsultation: '10 mai 2026', statut: 'Nouveau'  },
  { id: '#P-2104', nom: 'Zahraoui',  prenom: 'Fatima',  age: 58, telephone: '06 99 00 11 22', email: 'f.zahraoui@mail.com', medecinRef: 'El Harrab', derniereConsultation: '08 mai 2026', statut: 'Suivi'   },
  { id: '#P-7742', nom: 'Doukkali',  prenom: 'Karim',   age: 49, telephone: '06 22 44 66 88', email: 'k.doukkali@mail.com', medecinRef: 'Benali',    derniereConsultation: '05 mai 2026', statut: 'Suivi'   },
  { id: '#P-3301', nom: 'Chraibi',   prenom: 'Nadia',   age: 63, telephone: '06 33 55 77 99', email: 'n.chraibi@mail.com',  medecinRef: 'Ibrahim',   derniereConsultation: '02 mai 2026', statut: 'Clôturé' },
  { id: '#P-8840', nom: 'Bensalah',  prenom: 'Amina',   age: 31, telephone: '06 77 88 99 00', email: 'a.bensalah@mail.com', medecinRef: 'Ibrahim',   derniereConsultation: '01 mai 2026', statut: 'Nouveau'  },
  { id: '#P-1193', nom: 'El Fassi',  prenom: 'Omar',    age: 55, telephone: '06 44 55 66 77', email: 'o.elfassi@mail.com',  medecinRef: 'El Harrab', derniereConsultation: '28 avr 2026', statut: 'Suivi'   },
  { id: '#P-6628', nom: 'Moussaoui', prenom: 'Laila',   age: 47, telephone: '06 12 23 34 45', email: 'l.mouss@mail.com',    medecinRef: 'Benali',    derniereConsultation: '25 avr 2026', statut: 'Suivi'   },
];

const MEDECINS_FILTRE = ['Tous', 'El Harrab', 'Benali', 'Ibrahim'];
const STATUTS_FILTRE  = ['Tous', 'Suivi', 'Nouveau', 'Clôturé'];
const PAR_PAGE = 6;
...
export default function SecretairePatients() {
  ...
  const handleSave = (form) => {
    const nouveau = {
      ...
      medecinRef: form.medecinRef, derniereConsultation: '—', statut: 'Nouveau',
    };
  ...
  return (
    <div className="space-y-5 pb-8">
      ...
      {/* ── Stats ── */}
      <motion.div {...fadeUp(0.05)} className="grid grid-cols-2 xl:grid-cols-4 gap-3">
        <StatCard icon={Users}      label="Total patients"  value={patients.length}                                        color="pink"  />
        <StatCard icon={UserPlus}   label="Nouveaux"        value={patients.filter(p => p.statut === 'Nouveau').length}    color="pink"  />
        <StatCard icon={CalendarDays} label="En suivi"      value={patients.filter(p => p.statut === 'Suivi').length}      color="dark"  />
        <StatCard icon={TrendingDown} label="Clôturés"      value={patients.filter(p => p.statut === 'Clôturé').length}    color="amber" />
      </motion.div>
          ...
          >
            <div className="flex flex-wrap gap-4 p-4 bg-white border border-gray-100 rounded-2xl shadow-sm">
              <div className="flex flex-col gap-1.5 min-w-[160px]">
                ...
                <select value={filtreMedecin} onChange={(e) => { setFiltreMedecin(e.target.value); setPage(1); }}
              ...
              {filtersActifs && (
                <div className="flex items-end">
                  <button onClick={() => { setFiltreMedecin('Tous'); setFiltreStatut('Tous'); setPage(1); }}
              ...
              {Array.from({ length: totalPages }, (_, i) => i + 1).map((n) => (
                <button key={n} onClick={() => setPage(n)}
                  className={`w-8 h-8 rounded-lg text-[12px] font-semibold transition-colors ${
                    n === page ? 'bg-black text-white' : 'border border-gray-200 text-gray-600 hover:bg-gray-50'
                  }`}>
                  {n}
                </button>
              ))}
      ...
      {/* Modal */}
      <AnimatePresence>
        {showModal && <ModalCreerPatient onClose={() => setShowModal(false)} onSave={handleSave} />}
      </AnimatePresence>
    </div>
  );
}